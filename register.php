...
if(isset($_POST['submit'])){
	...
	if(strlen($_POST['username']) < 3){
		...
		if(!empty($row['username'])){
			$error[] = 'Username provided is already in use.';//@todo Translate
		}

	}

	if(strlen($_POST['password']) < 3){
				$error[] = 'Password is too short.';//@todo Translate
	}

	if(strlen($_POST['passwordConfirm']) < 3){
		$error[] = 'Confirm password is too short.';//@todo Translate
	}

	if($_POST['password'] != $_POST['passwordConfirm']){
		$error[] = 'Passwords do not match.';//@todo Translate
	}
	...
	if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
		...
		if(!empty($row['email'])){
			$error[] = 'Email provided is already in use.';//@todo Translate
		}
